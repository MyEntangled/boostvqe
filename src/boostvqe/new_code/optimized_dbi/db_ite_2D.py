...
import numpy as np
#from create_2d_hamiltonian import create_2d_hamiltonian_mpo

def create_2d_hamiltonian_mpo(Lx:int, Ly:int, Jx:float, Jy:float, Jz:float):
    L = Lx * Ly
    coor_to_idx = lambda x, y: x * Ly + y
    idx_to_coor = lambda idx: (idx // Ly, idx % Ly)
    ...
    for interaction_type, op, coupling_strength in zip(['X', 'Y', 'Z'], [X, Y, Z], [Jx, Jy, Jz]):

        W = np.zeros([d, d, 2, 2], dtype=complex)

        W[0, 0, :, :] = I
        W[0, 1, :, :] = op
        ## Absorb horizontal interaction
        W[1, Sf, :, :] = op

        ## Long-range Vertical interactions
        for y in range(1, Ly):
            W[y, y + 1, :, :] = I

        W[Ly, Sf, :, :] = op  # absorbing state for vertical interactions

        W[Sf, Sf, :, :] = I

        # define the end pieces
        Wl = W[0, :, :, :]
        Wr = W[:, Sf, :, :]

        W_edge = W.copy()
        W_edge[1, Sf, :, :] = 0 * op  # cancel consecutive edge between nodes on different rows
        ...
        H += coupling_strength * qtn.MatrixProductOperator(tensor_list)
...
if __name__ == "__main__":
    ...
    Lx, Ly = 4,4
    Jx, Jy, Jz = 0.25, 0.25, 0.125
    n_sites = Lx * Ly
    max_bond = 256
    simulate_steps = 50
    init = 'dmrg' # 'dmrg' or 'hadamard'
    filename = f'simulate_db_ite_L{Lx}_{Ly}_{init}.pkl'

    H = create_2d_hamiltonian_mpo(Lx, Ly, Jx, Jy, Jz)
    ...
    try:
        ...
        record = {'Lx': Lx, 'Ly': Ly, 'n_sites': n_sites, 'E_exact': exact_energy, 'psi_exact': exact_state,
